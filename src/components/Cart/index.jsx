...
export const CartComponent = () =>{
    ...
    // function Cart() {
        ...
        function submitOrder(){
            ...
            const order={
                buyer:{...form},
                item:CartContext,
                date: firebase.getFirestoreDB.Timestamp.fromDate(new Date()),
                total: total,
            }
            orders.add(order)
    ...
    <div className="cartItems">
        {cart.length>0 && cart.map(product =><ItemComponent addToCart={product.addToCart} key={product.id} id={product.id} title={product.title} categoryId={product.categoryId} image={product.image} description={product.description} stock={product.stock} price={product.price}/>)}
    </div>
